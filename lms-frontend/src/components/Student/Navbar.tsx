...
import { getGamificationProfile, recordLoginStreak } from "../../services/api/gamification";
...
const Navbar = () => {
    ...
    const fetchUser = async () => {
        ...
        if (stored) {
            ...
            try {
                ...
                await recordLoginStreak(uname);
                const profile = await getGamificationProfile(uname);
                setLevel(profile.level);
                setXp(profile.xpPoints);
                setStreak(profile.currentStreak);
            } catch (err) {
                console.error("Erreur gamification/details", err);
                setFullName(uname);
            }
        }
    };
    ...
    return (
        <nav className="main-header navbar navbar-expand-xl" style={{
            ...
            padding: '0.5rem 1rem',
            marginLeft: 0 // If it's a top nav without sidebar, remove AdminLTE left margin
        }}>
            <div className="container-fluid px-2 px-md-4">
                <Link to={"/student"} className="navbar-brand d-flex align-items-center mr-4" style={{ gap: '0.75rem' }}>
                    ...
                    <span style={{ color: 'var(--text-primary)', fontWeight: 800, letterSpacing: '1px', fontSize: '1.1rem' }}>
                        <span style={{ color: 'var(--green)', fontWeight: 400, opacity: 0.9 }}>ÉTUDIANT</span>
                    </span>
                </Link>
                ...
                <div className="collapse navbar-collapse order-3" id="navbarCollapse">
                    <ul className="navbar-nav mx-auto" style={{ gap: '0.2rem' }}>
                        <li className="nav-item">
                            <Link to={"/student"} style={navLinkStyle("/student")} className="nav-link-htb">
                                <i className="fas fa-home" style={{ width: 16, textAlign: 'center' }}></i> Accueil
                            </Link>
                        </li>
                        <li className="nav-item">
                            <Link to={"/student/messages"} style={{ ...navLinkStyle("/student/messages"), position: 'relative' }} className="nav-link-htb">
                        ...
                        <li className="nav-item">
                            <Link to={"/student/announcements"} style={navLinkStyle("/student/announcements")} className="nav-link-htb">
                                <i className="fas fa-bullhorn" style={{ width: 16, textAlign: 'center' }}></i> Annonces
                            </Link>
                        </li>
                        <li className="nav-item">
                            <Link to={"/student/grades"} style={navLinkStyle("/student/grades")} className="nav-link-htb">
                                <i className="fas fa-star" style={{ width: 16, textAlign: 'center' }}></i> Notes
                            </Link>
                        </li>
                        <li className="nav-item">
                            <Link to={"/student/mentorship"} style={navLinkStyle("/student/mentorship")} className="nav-link-htb">
                                <i className="fas fa-hands-helping" style={{ width: 16, textAlign: 'center' }}></i> Mentorat
                            </Link>
                        </li>
                        <li className="nav-item">
                            <Link to={"/student/certificates"} style={navLinkStyle("/student/certificates")} className="nav-link-htb">
                                <i className="fas fa-graduation-cap" style={{ width: 16, textAlign: 'center' }}></i> Certificats
                            </Link>
                        </li>
                        <li className="nav-item">
                            <Link to={"/student/analytics"} style={navLinkStyle("/student/analytics")} className="nav-link-htb">
                                <i className="fas fa-chart-line" style={{ width: 16, textAlign: 'center' }}></i> Analyses
                            </Link>
                        </li>
                    </ul>
